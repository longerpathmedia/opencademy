...
// --- Title helper ---
if (typeof window.COURSE_TITLE !== 'undefined') {
...
// --- Course experience ---
const modulesHost = document.getElementById('modules');
const hasCourseData = modulesHost && typeof window.MODULES !== 'undefined';

if (hasCourseData) {
  const modulesEl = modulesHost;
  const modulesData = window.MODULES;
  ...
  function computeStats() {
    ...
    if (lessonCountEl) lessonCountEl.textContent = `${totalLessons} lesson${totalLessons === 1 ? '' : 's'}`;
    if (totalDurationEl) totalDurationEl.textContent = totalSeconds ? `~${formatSeconds(totalSeconds)} total` : 'Total time varies';
  }

  function render() {
    modulesEl.innerHTML = '';
    modulesData.forEach((module, mi) => {
  ...
  function openVideo(lessonId) {
    const lesson = modulesData.flatMap(m => m.lessons).find(l => l.id === lessonId);
    if (!lesson || !modal || !player || !playerTitle || !markDoneBtn) return;
  ...
  if (markDoneBtn) {
    markDoneBtn.addEventListener('click', () => {
      ...
      const value = !getDone(currentLessonId);
      setDone(currentLessonId, value);
      markDoneBtn.textContent = value ? 'Completed ✓' : 'Mark complete';
      render();
    });
  }
  ...
  if (resetProgressBtn) {
    resetProgressBtn.addEventListener('click', () => {
      ...
      modulesData.forEach(m => m.lessons.forEach(l => localStorage.removeItem(key(l.id))));
      render();
    });
  }
